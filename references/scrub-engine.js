/* ============================================================================
   ...
   The engine loads each clip via same-origin `<video src>` (CSP-safe; no blob: URLs) and scrubs
   currentTime. Encoded assets should be served with HTTP Range support (+faststart).
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  const SEAM_TRIM_FRAMES = Math.max(0, Number(config.seamTrimFrames) || 0);
  ...
  hintText.textContent = config.loadingHint || 'загрузка…';
  ...
  // Off-screen decode host: clips warm here so opacity:0 scenes still decode (late legs).
  const preloadRoot = el('div', 'sw-preload');
  preloadRoot.style.cssText = 'position:fixed;left:0;top:0;width:1px;height:1px;overflow:hidden;clip-path:inset(50%);pointer-events:none;z-index:-1';
  container.appendChild(preloadRoot);
  ...
  const initialHint = config.hint || 'scroll';
  let firstClipLive = false;
  let backgroundPreloadsStarted = false;
  ...
  function layout() {
    ...
    // The videos are the primary content of Scroll World, so reduced motion
    // must not replace the experience with a still-image slideshow.
    if (NSEG) loadClip(SEGMENTS[0]);
    scheduleRead();
  }
  ...
  function markClipPainted(s) {
    if (!s.el || !s.video || s.video.parentNode !== s.el) return;
    if (s.video.readyState < 2) return;
    s.el.classList.add('has-clip');
    if (s === SEGMENTS[0] && !firstClipLive) {
      firstClipLive = true;
      hintText.textContent = initialHint;
    }
  }

  function mountClip(s) {
    if (!s.video || s.video.parentNode === s.el) return;
    s.el.appendChild(s.video);
  }

  function revealClip(s) {
    mountClip(s);
    markClipPainted(s);
  }

  function startBackgroundPreloads() {
    if (backgroundPreloadsStarted) return;
    backgroundPreloadsStarted = true;
  }

  function loadClip(s) {
    // Videos are the primary content, not decorative motion. Reduced motion
    // still disables particles and smooth navigation, but must not disable scrubbing.
    if (!s.clip || s.hasClip) return;
    if (s.loading) return;
    s.loading = true;
    ...
    // Same-origin URL (not blob:) so strict portfolio CSP (default-src 'self') still works.
    v.src = url;
    const onProgress = () => revealClip(s);
    ...
    v.addEventListener('seeked', () => { onProgress(); scheduleRead(); }, { once: true });
    ...
    v.addEventListener('canplay', () => { onProgress(); scheduleRead(); });
    v.addEventListener('error', () => { s.loading = false; s.hasClip = false; s.video = null; });
    // Leg 0 decodes in the visible scene; later legs warm off-screen until mounted.
    const host = s === SEGMENTS[0] ? s.el : preloadRoot;
    host.appendChild(v);
    s.video = v; s.hasClip = true;
  }

  // Dive legs share exact scroll boundaries. Hard cut at the seam — no cross-dissolve.
  function computeSegmentOpacities(y, fade) {
    const ops = new Array(NSEG).fill(0);

    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (y < s.start - fade || y > s.end + fade) continue;

      if (s.kind === 'conn') {
        let outside = 0;
        if (y < s.start) outside = s.start - y;
        else if (y > s.end) outside = y - s.end;
        ops[i] = smooth(1 - outside / fade);
        continue;
      }

      const next = SEGMENTS[i + 1];
      const sharedEnd = next && next.kind === 'dive' && next.start === s.end;

      if (y >= s.start && y < s.end) {
        ops[i] = 1;
      } else if (sharedEnd && y >= s.end && y < s.end + fade) {
        // Outgoing tail only when there is no incoming dive yet at this y.
        ops[i] = 0;
      } else if (y < s.start) {
        ops[i] = smooth((y - (s.start - fade)) / fade);
      } else if (!sharedEnd && y > s.end) {
        ops[i] = smooth((s.end + fade - y) / fade);
      }
    }
    ...
    for (let i = 0; i < NSEG - 1; i++) {
      ...
      if (a.kind !== 'dive' || b.kind !== 'dive' || a.end !== b.start) continue;
      const seam = a.end;
      if (y >= seam) {
        ops[i] = 0;
        ops[i + 1] = Math.max(ops[i + 1], 1);
      } else {
        ops[i] = Math.max(ops[i], 1);
        ops[i + 1] = 0;
      }
    }
  ...
  function read() {
    ...
    const segmentOps = computeSegmentOpacities(y, fade);

    const lookAhead = 2.5 * vh;
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (s.hasClip) mountClip(s);
      if (y > s.start - lookAhead && y < s.end + lookAhead) loadClip(s);
      const local = clamp((y - s.start) / (s.end - s.start), 0, 1);
      s.target = s.linger ? lingerEase(local, s.linger) : local;
      const op = segmentOps[i];
    ...
    for (let i = 0; i < N; i++) {
      ...
      const fadeIn = 0.12;
      const fadeOut = Math.min(0.18, Math.max(CROSSFADE / (seg.w || DIVE_W), 0.06));
      let cop;
      if (before) {
        cop = 0;
      } else if (after) {
        ...
        // Intro: visible on landing, hold through the still, fade only at the next leg seam.
        cop = 1;
        if (pr > 1 - fadeOut) cop = smooth((1 - pr) / fadeOut);
      } else if (i === N - 1) {
        // Finale: fade in at section start, then hold (CTA).
        cop = pr < fadeIn ? smooth(pr / fadeIn) : 1;
      } else {
        // Leg sections: fade in at start, hold for the full clip, fade out only when the next leg begins.
        cop = 1;
        if (pr < fadeIn) cop = smooth(pr / fadeIn);
        else if (pr > 1 - fadeOut) cop = smooth((1 - pr) / fadeOut);
      }
      const c = copies[i];
      c.style.opacity = cop;
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      const seamTrim = s.kind === 'dive' && s.si < N - 1
        ? SEAM_TRIM_FRAMES / 24
        : 0;
      const t = Math.min(
        clamp(s.cur, 0, 0.999) * dur,
        Math.max(0, dur - seamTrim),
      );
...
function injectCSS() {
  ...
  /* Portrait phones: keep the same centre anchor as desktop to avoid seam drift. */
