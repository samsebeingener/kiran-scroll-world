#!/usr/bin/env python3
"""Extract last frame from a video leg MP4 (for chaining Seedance legs)."""
...
from video_frame_chain import extract_last_frame, last_frame_path_for_leg


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--video", required=True, type=Path, help="Source MP4 (NNN-leg-LL.mp4)")
    parser.add_argument("--out", type=Path, default=None, help="Output PNG (default: assets/frames/NNN-leg-LL-last.png)")
    ...
    if args.out:
        ...
        dest = last_frame_path_for_leg(args.project.resolve(), ver, args.leg)

    out = extract_last_frame(video, dest)
    print(out)
